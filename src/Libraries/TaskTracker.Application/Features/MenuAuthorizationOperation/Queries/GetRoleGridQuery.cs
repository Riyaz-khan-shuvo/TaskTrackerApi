...
{
    public class GetRoleGridQuery : GridOptions, IRequest<ResultVM>
    {

        public class Handler : IRequestHandler<GetRoleGridQuery, ResultVM>
        {
            ...
            public async Task<ResultVM> Handle(GetRoleGridQuery request, CancellationToken cancellationToken)
            {
                var gridData = await _menuAuthorizationRepository.GetRoleGridData(request);
